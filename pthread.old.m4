...
m4_define(SPLASH4_ROI_BEGIN, `__asm__ volatile("srai zero, zero, 0")')
m4_define(SPLASH4_ROI_END, `__asm__ volatile("srai zero, zero, 1")')

m4_dnl Region markers
m4_define(_NOTE_START_LOCK, `RMS_Initial_Acq')
m4_define(_NOTE_END_LOCK, `RMS_Final_Acq')
...
m4_define(FETCH_ADD, `({_NOTE_START_ATOMIC(); uint64_t ___x =atomic_fetch_add((_Atomic __typeof__(*(&($1))) *)(&(($1))), $2); _NOTE_END_ATOMIC(); ___x;});')
m4_define(FETCH_SUB, `({_NOTE_START_ATOMIC(); uint64_t ___x =atomic_fetch_sub((_Atomic __typeof__(*(&($1))) *)(&(($1))), $2); _NOTE_END_ATOMIC(); ___x;});')
m4_define(STORE,`atomic_store((_Atomic __typeof__(*(&($1))) *)(&(($1))), $2)')
m4_define(LOAD,`atomic_load((_Atomic __typeof__(*(&($1))) *)(&(($1))))')
m4_define(CAS, `({_NOTE_START_CMPXCHG(); _Bool ___b = atomic_compare_exchange_weak((_Atomic __typeof__(*(&($1))) *)(&(($1))), &($2), $3); _NOTE_END_CMPXCHG(); ___b;})')
...
if (atomic_fetch_sub((_Atomic __typeof__(*(&(__count__))) *)(&((__count__))), 1) == 1) {
	__count__ = $2;
